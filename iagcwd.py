...
def image_agcwd(img: np.ndarray, a: float = 0.25, truncated_cdf: bool = False) -> np.ndarray:
    # h, w = img.shape[:2]
    hist, bins = np.histogram(img.flatten(), 256, [0.0, 256.0], density=True)
    cdf = hist.cumsum()
...
def process_bright(img: np.ndarray) -> np.ndarray:
...
def process_dimmed(img: np.ndarray) -> np.ndarray:
    agcwd = image_agcwd(img, a=0.75, truncated_cdf=True)
    return agcwd


def prepare_image(name:str, img: np.ndarray) -> np.ndarray:
